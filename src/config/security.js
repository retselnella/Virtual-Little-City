...
// Multiplayer talks to Supabase Realtime (HTTPS for anonymous sign-in, WSS for channels), and music streams from its Storage. Browsers enforce every policy
...
// Music plays from the Supabase project's public Storage bucket (HTTPS only), so media follows the same hosts.
export function contentSecurityPolicy(connect = SUPABASE_HOSTS) {
  return [
    ...
    ["media-src 'self'", ...connect.filter(origin => origin.startsWith('https://'))].join(' '),
