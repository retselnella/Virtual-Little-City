...
import { regionAt } from '../models/worldTour/worldIsland.js';
...
export function useWorldController(character = null, suspended = false) {
  ...
  useEffect(() => {
    ...
    let dispose;
    try { dispose = mountAdventure(host.current, session, input, paused, s => { setHud(snapshot(s)); save(s); setReady(true); }, () => setError(true), online.roster, environment); }
  ...
  const playerName = displayName(character), region = regionAt(p.x, p.z, city.district), weather = weatherLabel(sky);
  return { online, playerName, sky, weather, region, hud, panel, ready, error, storage, host, city, current, p, point, open, action, toggleBlood, touchControl, travel, dispatchTitle, dispatchHint, task, blood, acceptContract, abandonContract, recoverToSafehouse };
}
