...
import { CITIES, CONTRACTS, generateBlocks, createSession, cleanWorldSave, freePosition, clearSight, startContract, interact, toggleVehicle, attack, stepWorld, recover } from '../../src/models/worldTour/worldAdventure.js';
...
test('combat respects line of sight, range, cooldown, ammunition and enemy death', () => {
  ...
  attack(s); assert.equal(s.enemies[0].health, 60); assert.equal(s.ammo, 47); assert.equal(s.heat, 1);
  attack(s); assert.equal(s.ammo, 47);
  ...
  s.ammo = 0; s.cooldown = 0; attack(s); assert.equal(s.ammo, 0);
...
test('death respawns, cancels the contract and keeps earned rewards; reload restores ammunition', () => {
  ...
  s.ammo = 0; s.reload = 1.5; for (let i = 0; i < 31; i++) stepWorld(s, {}, 0.05); assert.equal(s.ammo, 48);
  recover(s); assert.equal(s.cash, 650);
});
test('saves sanitize invalid data and keep city-specific completion', () => {
  assert.deepEqual(cleanWorldSave(null), { city: 'miami', cash: 0, completed: [] });
  assert.deepEqual(cleanWorldSave({ city: 'invalid', cash: -50, completed: ['miami:courier', 'miami:courier', 'unknown'] }), { city: 'miami', cash: 0, completed: ['miami:courier'] });
